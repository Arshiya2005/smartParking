...
export const ownerAreas = async (req, res) => {
    try {
        ...
        const area = await sql`
            SELECT * FROM parkingspot WHERE is_active = 'true' AND owner_id = ${id};
        `;
...
export const areaActiveBookings = async (req, res) => {
    try {
        ...
        const response = await sql`
            SELECT * FROM bookings WHERE slot_id = ${id} AND date = ${today} AND status = 'active' ORDER BY sTime ASC
        `;
...
export const areaBookingHistory = async (req, res) => {
    try {
        ...
        const id = req.query.id;
        const response = await sql`
            SELECT * FROM bookings WHERE slot_id = ${id} ORDER BY sTime ASC
        `;
        if (response.length > 0) {
            return res.status(200).json({ data : response});
        }
