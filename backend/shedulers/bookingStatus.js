  ...
  for (const b of bookings) {
    ...
    if (userSocketId) {
      io.to(userSocketId).emit('booking-reminder', {
        message: `Your parking starts at ${b.stime} for ${b.name}`,
      });
      await sql`
        INSERT INTO notifications (users_id, message, created_at, status)
        VALUES (${b.customer_id}, ${`Your booking at ${b.stime} for ${b.name} is about to start.`}, ${now}, 'read')
      `;
      console.log(`Reminder sent to user ${b.customer_id}`);
    } else {
      await sql`
        INSERT INTO notifications (users_id, message, created_at, status)
        VALUES (${b.customer_id}, ${`Your booking at ${b.stime} for ${b.name} is about to start.`}, ${now}, 'unread')
      `;
